...
def count(interval):
    global objects, old_objects, jumlahIkan, object_movements, frame_rate_calc, obsFrames
    ...
    while(video.isOpened()): # Uncomment block for recorded video input
        ...
        if (current_time - start_time >= interval):
            ...
            print("Jumlah ikan yg lewat: {}".format(len(jumlahIkan)))

            # Menampilkan jumlah ID pada frame
            cv2.putText(frame, f'Total Bibit: {len(jumlahIkan)}', (30, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2, cv2.LINE_AA)
